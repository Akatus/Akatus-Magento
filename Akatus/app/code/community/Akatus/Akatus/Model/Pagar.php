	...
	public function GeraXML(Varien_Object $payment, $amount){
		...
		<carrinho>
			...
			<transacao>
				...
				<!-- Dados do checkout -->
				<moeda>BRL</moeda>
				<frete_total>'.number_format($order->base_shipping_incl_tax,'2','','').'</frete_total> 
				<desconto_total>'.number_format($order->shipping_discount_amount,'2','','').'</desconto_total> 
		...
		*/
		...
		else
		{
			...
			if($data["resposta"]["status"]["value"]=="Em Análise"){
				...
				// Mage::getSingleton('checkout/session')->addSuccess(Mage::helper('checkout')->__("tudo ok"));
				} catch (Exception $e){
					$e->getMessage();
